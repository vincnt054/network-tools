...
func TestSimpleHTTPServer(t *testing.T) {
	srv = &http.Server{
		Addr: "127.0.0.1:8081",
		Handler: http.TimeoutHandler( // passed through middleware
			handlers.DefaultHandler(), 2*time.Minute, ""), // passed through handler
		IdleTimeout: 5 * time.Minute, // waiting for the the next client request when the communication uses keepalives
		ReadHeaderTimeout: time.Minute, // how long the server will wait to finish reading the request ehaders
	}
